    ...
    public static void updateMouseOversSkinned(float elapsedTime) {
        if (Minecraft.getMinecraft().player != null) {
            ...
            if (Minecraft.getMinecraft().player.getName().contains("rich1051414") && Minecraft.getMinecraft().player.isSneaking()) {
                Entity tmp = RaytraceUtil.getClosestEntity(Minecraft.getMinecraft().player, DIConfig.mainInstance().mouseoverRange);
            ...
            if (el != null || LastTargeted != 0 && (DIConfig.mainInstance().portraitLifetime == -1 || tick > 0.0D)) {
    ...
    private void updateHealth(EntityLivingBase el, int currentHealth) {
        if (healths.containsKey(el.getEntityId())) {
            ...
            if (lastHealth != currentHealth) {
                ...
                DIWordParticles customParticle = new DIWordParticles(Minecraft.getMinecraft().world, el.posX, var10004, el.posZ, 0.001D, 0.05F * DIConfig.mainInstance().BounceStrength, 0.001D, damage);
