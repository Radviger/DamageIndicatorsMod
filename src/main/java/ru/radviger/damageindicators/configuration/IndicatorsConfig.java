package ru.radviger.damageindicators.configuration;
...
public class IndicatorsConfig {
    private static IndicatorsConfig config;
    ...
    private IndicatorsConfig(File file, int check) {
    ...
    public static void loadConfig(File file) {
        if (file == null) {
            config = new IndicatorsConfig(mainInstance().CONFIG_FILE, 0);
        } else {
            config = new IndicatorsConfig(file, 0);
        }

    }

    public static IndicatorsConfig useNewConfig(IndicatorsConfig newConfig) {
        config = newConfig;
        return mainInstance();
    }

    public static void overrideConfigAndSave(IndicatorsConfig newConfig) {
        try {
    ...
    private void loadConfig() {
        try {
            ...
            if (flag && !config.get("Internal", "version", "0").getString().equals("1")) {
                overrideConfigAndSave(new IndicatorsConfig(this.CONFIG_FILE, 1));
                return;
            }
